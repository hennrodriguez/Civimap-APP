<?php
	...
	$sql = "SELECT id, suceso_titulo, suceso_descripcion, suceso_ubicacion FROM sucesos WHERE id='$suceso_id'";
	$result = $conectar->query($sql);
	if($result->num_rows > 0){
		while($row = $result->fetch_assoc()){
			echo "<a href='suceso.html' class='mdl-card mdl-shadow--2dp suceso-id__" . $row["id"]. "'>
		...
		echo "No encontramos esta actualización.";
	}
